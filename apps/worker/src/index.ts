...
const performTcpPing = async (host: string, port: number) => {
  ...
  try {
    ...
    await fetch(`http://${host}:${port}`, { method: 'HEAD', signal: controller.signal as any });
  ...
  try {
    ...
    for (let i = 0; i < nodes.length; i++) {
      ...
      await telemetryQueue.add(
        'probe',
        { id: node.id, hostname: node.hostname, port: node.port, protocol: node.protocol },
        { delay: i * delayStep },
      );
    }
...
async function bootstrap() {
  ...
  logger.info(
    { nodeProbeIntervalMs: NODE_PROBE_INTERVAL_MS, providerSyncIntervalMs: PROVIDER_SYNC_INTERVAL_MS },
    'worker bootstrapped — starting schedulers',
  );
